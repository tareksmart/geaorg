...
namespace sales_pro
{
    public partial class ag_acc_frm : Form
    {
        public ag_acc_frm()
        {
            InitializeComponent();
        ...
        private void mor_pay_quer_exe_btn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (mor_pay_quer_nam_cmbx.Text != "")
                {
                    ...
                    select_ag_dis_cmd.CommandText = "select AgentDetailId,AgentDetailDate, AgentMny, AgentNotes from AgentDetails" +
                        ...
                        "' and del_flag<>'del' and ag_mor_flag='Ag' and AgentId=" +

                     mor_pay_quer_nam_cmbx.SelectedValue.ToString();
        ...
        private void ag_acc_frm_Load(object sender, System.EventArgs e)
        {
            ...
            {
                mor_pay_name_cmbx.DataSource = meth.select_all_agent();
                mor_pay_quer_nam_cmbx.DataSource = meth.select_all_agent();
            }
        }
        ...
        private void mor_pay_new_btn_Click(object sender, System.EventArgs e)
        {
            try
            {
                if (pay_op_code_bx.Text == "")
                {
                    ...
                    {
                        string result = "";
                        result = meth.minus_mny_ag_account(mor_pay_name_cmbx.SelectedValue.ToString(), mor_pay_mny_bx.Text.Trim());
                        ...
                        insert_mor_pay_cmd.CommandText = "insert into AgentDetails(AgentId, AgentStock, AgentMny, AgentNotes, AgentDetailDate, ag_mor_flag" + ")values(" +
                            ...
                            + mor_pay_notes_bx.Text + "','" + mor_pay_date.Text + "','Ag" + "')";
                        connect_sal.Open();
                        insert_mor_pay_cmd.ExecuteNonQuery();
                        connect_sal.Close();

                        if (meth.select_ag_data_bynam_orcode(mor_pay_name_cmbx.SelectedValue.ToString(), "null","Ag").Rows.Count > 0)
                        {
                            mor_pay_remain_bx.Text = meth.select_ag_data_bynam_orcode(mor_pay_name_cmbx.SelectedValue.ToString(), "null", "Ag").Rows[0][2].ToString();
                        }
                ...
                try
                {
                    ...
                    if (mor_pay_mny_bx.Text.Trim() != "")
                    {
                        ...
                        if (mor_pay_quer_grid.Rows.Count > 0)
                            meth.add_mny_ag_account(mor_pay_name_cmbx.SelectedValue.ToString(), mor_pay_quer_grid.CurrentRow.Cells[2].Value.ToString());

                        SqlCommand delete_pay_cmd = connect_sal.CreateCommand();

                        delete_pay_cmd.CommandText = "delete from AgentDetails" +
                            " WHERE AgentDetailId=" + pay_op_code_bx.Text.Trim() + " and ag_mor_flag='Ag'";

                        connect_sal.Open();
                        int rows = delete_pay_cmd.ExecuteNonQuery();
                        connect_sal.Close();

                        meth.minus_mny_ag_account(mor_pay_name_cmbx.SelectedValue.ToString(), mor_pay_mny_bx.Text.Trim());//خصم من العميل
                        ...
                        ///////////تسجيل الخصم والرصيد الجديد من حساب العميل//////
                        SqlCommand insert_mor_pay_cmd = connect_sal.CreateCommand();
        ...
        private void mor_pay_del_btn_Click(object sender, System.EventArgs e)
        {
            if (pay_op_code_bx.Text != "")
            {
                try
                {
                    ...
                    meth.add_mny_ag_account(mor_pay_name_cmbx.SelectedValue.ToString(), mor_pay_mny_bx.Text);
                    // meth.add_mny_ag_account(mor_pay_name_cmbx.SelectedValue.ToString(), mor_pay_quer_grid.CurrentRow.Cells[2].Value.ToString());
                    ...
                    mor_pay_remain_bx.Text = meth.select_ag_data_bynam_orcode(mor_pay_name_cmbx.SelectedValue.ToString(), "null", "Ag").Rows[0][2].ToString();
                    //meth.select_ag_data_bynam_orcode(mor_pay_name_cmbx.SelectedValue.ToString(), "null","Ag").Rows.Count
                    mor_pay_clear_bx_btn.PerformClick();
        ...
        private void recalc_ag_btn_Click(object sender, System.EventArgs e)
        {
            try
            {
                double remain = 0;
                ...
                select_sum_sels_ag_cmd.CommandText = "select SUM(BillTotal) from Bill where s_or_c='c' and BillType='SEL' AND BillDate BETWEEN '" + recalc_start_date.Text + "' and '" +
                ...
                if (dtb1.Rows.Count > 0)
                {
                    if (dtb1.Rows[0][0].ToString().Trim() != "")
                        ag_sels = double.Parse(dtb1.Rows[0][0].ToString());
                }
                ...
                select_sum_pays_ag_cmd.CommandText = "SELECT SUM(Agentmny) from AgentDetails where ag_mor_flag ='Ag' and del_flag ='n' and AgentDetailDate between '" + recalc_start_date.Text + "' and '" +
                    recalc_end_date.Text + "' and AgentId=" + mor_pay_name_cmbx.SelectedValue.ToString();
                ...
                if (dtb2.Rows.Count > 0)
                {
                    if (dtb2.Rows[0][0].ToString().Trim() != "")
                        ag_pays = double.Parse(dtb2.Rows[0][0].ToString());
                }

                remain = ag_sels - ag_pays;
                ...
                if (MessageBox.Show("هل تريد تعديل حفظ هذاالرصيد" + "  " + remain.ToString(), "الرصيد المتبقى", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    ...
                    connect_sal.Close();
                    MessageBox.Show("لقد تم تعديل الرصيد النتبقى للعميل  " + mor_pay_name_cmbx.Text);
                }
            }
            catch (Exception hh)
            {
        ...
        private void recalc_ag_chbx_CheckedChanged(object sender, System.EventArgs e)
        {
            if (recalc_ag_chbx.Checked)
            {
                recalc_end_date.Visible = true;
                recalc_start_date.Visible = true;
                recalc_ag_btn.Visible = true;
                label9.Visible = true;
            }
            else
            {
